...
public class ReferController {
	...
	public String addSave(@ModelAttribute("referVO") ReferVO referVO, HttpSession session, ModelMap model) {
		UserVO user = (UserVO) session.getAttribute(Constants.SESSION_USER_KEY);
		referVO.setLastUpdator(user.getUserId());
		referVO.setLastUpdateDt(new Date());
	...
	public String editSave(@ModelAttribute("referVO") ReferVO referVO, String pageNo, HttpSession session, ModelMap model) {
		UserVO user = (UserVO) session.getAttribute(Constants.SESSION_USER_KEY);
		referVO.setLastUpdator(user.getUserId());
		referVO.setLastUpdateDt(new Date());
		referBO.saveoOrUpdate(referVO);
	...
	public String view(String referId, ModelMap model) {
		final ReferVO referVO = referBO.findById(referId);
		model.addAttribute("referVO", referVO);
		Map<String, String> zxlxDictMap = dictBO.getDict(Constants.DICT_DICTTYPE_ZXLX);
		model.addAttribute("zxlxDictMap", zxlxDictMap);
		
		if(StringUtil.isNull(referVO.getLastUpdator())) {
			model.addAttribute("lastUpdator", "-");
		} else {
			UserVO userVO = userBO.findById(referVO.getLastUpdator());
			model.addAttribute("lastUpdator", userVO.getUserName());
		}
		return "refer/view";
	}
