      ...
      <td>
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="1">
          ...
          <tr class="table_t1">
            <td width="10%">角色名称：</td>
            <td>
              <input name="roleName" size="70" class="form" value="${roleVO.roleName}"/>
            </td>
          </tr>
          <tr class="table_t1">
            	...
            	<c:forEach items="${operationList}" var="operation">
            		<c:choose>
            			<c:when test="${not empty operationIdMap[operation.operationId]}">
            				<input type="checkbox" id="operationIds" name="operationIds" checked="checked" value="${operation.operationId}" />${operation.operationName}</br>
            			</c:when>
            			<c:otherwise>
            				<input type="checkbox" id="operationIds" name="operationIds" value="${operation.operationId}" />${operation.operationName}</br>
            			</c:otherwise>
            		</c:choose>
            	</c:forEach>
        	</td>
